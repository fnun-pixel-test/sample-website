...
// Prepare contents for Facebook Pixel
const pixelContents = cartItems.map(item => ({
    id: item.id,
    name: item.name,
...
// Track Facebook Pixel Purchase event with detailed information
fbq('track', 'Purchase', {
    value: Number(orderAmount).toFixed(2),
    currency: 'USD',
    content_type: 'product',
    contents: pixelContents.length > 0 ? pixelContents : [{id: '0', quantity: 0}]
});
